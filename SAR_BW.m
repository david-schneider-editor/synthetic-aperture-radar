...
max_range = (3E8*size(S_st,2)/(2*bw))*1/.3048;
	...
	dr_index1 = round((dr1/max_range)*size(S_image,2));
    dr_index2 = round((dr2/max_range)*size(S_image,2));
	disp('dr indices:');
	disp(dr_index1);
	disp(dr_index2);
    cr_index1 = round(( (cr1+zpad*delta_x/(2*.3048)) /(zpad*delta_x/.3048))*size(S_image,1));
    cr_index2 = round(( (cr2+zpad*delta_x/(2*.3048))/(zpad*delta_x/.3048))*size(S_image,1));
	disp('cr indices:');
	disp(cr_index1);
	disp(cr_index2);
	...
	%trunc_image = S_image(cr_index1:cr_index2,7:2411);
	trunc_image = S_image(cr_index1:cr_index2,dr_index1:dr_index2);
	
	downrange = linspace(-1*dr1,-1*dr2, size(trunc_image,2)) + Rs/.3048;		
    crossrange = linspace(cr1, cr2, size(trunc_image, 1));

    %scale down range columns by range^(3/2), delete to make like
    %dissertation again

    clear ii;
    for ii = 1:size(trunc_image,1)
        trunc_image(ii,:) = (trunc_image(ii,:)').*(abs((downrange')*.3048)).^(3/2);
    end
    trunc_image = dbv(trunc_image); %added to scale to d^3/2
	
	trunc_image = flipud(trunc_image);

    imagesc([1 300], [-80 80], trunc_image, [max(max(trunc_image))-40, max(max(trunc_image))-0]);  
    ...
    cbar = colorbar;
    set(get(cbar, 'Title'), 'String', 'dB','fontsize',13); 
    %print(gcf, '-djpeg100', 'final_image2.jpg');
    if close_as_you_go == 1,
        close(figcount);
    end
    figcount = figcount + 1;
